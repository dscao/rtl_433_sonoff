...
static int fineoffset_wh1050_callback(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    */
    ...
    else if (bits == 78 && preamble_byte == 0x72) {
        fineoffset_wh1050_decode(decoder, bitbuffer, 6, TYPE_OOK);
    else if (bits == 77 && preamble_byte == 0xff) {
        fineoffset_wh1050_decode(decoder, bitbuffer, 5, TYPE_OOK);
    else if (bits == 76 && preamble_byte == 0xf5) {
        fineoffset_wh1050_decode(decoder, bitbuffer, 4, TYPE_OOK);
    else if (bits == 75 && preamble_byte == 0xeb) {
        fineoffset_wh1050_decode(decoder, bitbuffer, 3, TYPE_OOK);
    else if (bits == 74 && preamble_byte == 0xd6) {
        fineoffset_wh1050_decode(decoder, bitbuffer, 2, TYPE_OOK);
    else if (bits == 73 && preamble_byte == 0x2d) {
        fineoffset_wh1050_decode(decoder, bitbuffer, 1, TYPE_OOK);
